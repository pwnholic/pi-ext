...
export function createContainer(options: ContainerOptions = {}): Container {
    ...
    const inst: Instrumentation = { monitor: activity };
    ...
    const search = buildSearcher(new SearchService({ providers: [exa] }), inst, cache);
    const fetch = buildFetcher(new FetchService({ providers: [impers] }), inst, cache);
    const summarize = buildSummarizer(new SummarizeService({ llm: options.llm }), inst);
